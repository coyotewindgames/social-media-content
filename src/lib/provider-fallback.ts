/**
 * Provider Fallback Orchestration Module
 * 
...
async function generateWithOpenAI(
  ...
  const apiKey = options.apiKeys.openai || getEnvVar('OPENAI_API_KEY');
  if (!apiKey) {
...
async function generateWithClaude(
  ...
  const apiKey = options.apiKeys.anthropic || getEnvVar('ANTHROPIC_API_KEY');
  if (!apiKey) {
    throw new ProviderError(
...
/**
 * Get environment variable (works in both Node.js and browser environments).
 */
  ...
  try {
    switch (provider) {
      case Provider.OPENAI: {
        const apiKey =
          options?.apiKeys?.openai || getEnvVar('OPENAI_API_KEY');
      ...
      case Provider.CLAUDE: {
        const apiKey =
          options?.apiKeys?.anthropic || getEnvVar('ANTHROPIC_API_KEY');
        if (!apiKey) return false;
